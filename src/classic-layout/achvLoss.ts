...
type MapKey = Judgement | 'total';

function createEmptyJudgementMap(): Record<Judgement | 'total', number> {
    ...
    if (noteType === 'break') {
      ...
      dxNoteLoss.great =
        ...
        (breakDistribution.get(2000) * (1 - BREAK_BONUS_MULTIPLIER.get(2000)) +
          ...
          totalBreakCount;
      dxNoteLoss.good =
        (breakDistribution.get(1000) * (1 - BREAK_BASE_SCORE_MULTIPLIER.get(1000)) * baseScore) /
          scorePerPercentage +
        (breakDistribution.get(1000) * (1 - BREAK_BONUS_MULTIPLIER.get(1000))) / totalBreakCount;
      dxNoteLoss.miss =
      ...
      finaleNoteLoss.great = Math.round(
        judgements.great * baseScore * (1 - REGULAR_BASE_SCORE_MULTIPLIER.great)
      );
      finaleNoteLoss.good = Math.round(
        judgements.good * baseScore * (1 - REGULAR_BASE_SCORE_MULTIPLIER.good)
      );
